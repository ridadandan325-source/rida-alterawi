...
@section('title', 'تعديل سجل العلاج')

@section('content')
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">تعديل سجل العلاج</h1>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('treatment-notes.update', $treatmentNote) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('patient_id') is-invalid @enderror" 
                        ...
                        @foreach($patients as $patient)
                            <option value="{{ $patient->id }}" {{ old('patient_id', $treatmentNote->patient_id) == $patient->id ? 'selected' : '' }}>
                                {{ $patient->full_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('dentist_id') is-invalid @enderror" 
                        ...
                        @foreach($dentists as $dentist)
                            <option value="{{ $dentist->id }}" {{ old('dentist_id', $treatmentNote->dentist_id) == $dentist->id ? 'selected' : '' }}>
                                {{ $dentist->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('appointment_id') is-invalid @enderror" 
                        ...
                        @foreach($appointments as $appointment)
                            <option value="{{ $appointment->id }}" {{ old('appointment_id', $treatmentNote->appointment_id) == $appointment->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" 
                           ...
                           id="tooth_number" 
                           name="tooth_number" 
                           value="{{ old('tooth_number', $treatmentNote->tooth_number) }}"
            ...
            <div class="mb-3">
                ...
                <input type="text" 
                       ...
                       id="title" 
                       name="title" 
                       value="{{ old('title', $treatmentNote->title) }}" 
                       required>
                @error('title')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" 
                          ...
                          name="description" 
                          rows="6" 
                          required>{{ old('description', $treatmentNote->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
